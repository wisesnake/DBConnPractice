...
//		한 튜플당 하나씩 배열의 형태로 저장하기 위한 어레이리스트 제너릭 타입
//		리스트 객체인 memberList 객체 생성.
		...
		try {
			...
			//여기서 ResultSet타입은 저장된 값을 한 튜플 단위로 불러올 수 있음.
			
			
			//Set계열 데이터
			while(rs.next()) {
				//ResultSet.next() 메서드 : Moves the cursor forward one row from its current position
				//즉 한 행씩(ResultSet타입 변수에 저장된 테이블 데이터의 한 튜플씩) 커서를 이동시킴.
				String id = rs.getString("id");
				//ResultSet.getString
				//Retrieves the value of the designated column in the current rowof this ResultSet object as a String in the Java programming language.
				//즉, 현재 커서가 가르키고 있는 튜플에서, 매개값과 일치하는 문자열을 지닌 필드의 값을 뽑아 반환받음.
				...
				memberList.add(vo);
				//한 루프당 한 튜플의 모든 필드값을 저장한 vo객체를 위에서 만든 Arraylist배열의 
